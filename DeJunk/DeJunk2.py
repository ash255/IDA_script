...
def UI(item):
	...
	esta.insert("end","%X" % ScreenEA()) #默认为当前光标地址
...
def dejunk(start,length,dir,S,R):
	...
	if(dir == "0"):
		...
		while(1):
			start = FindBinary(start,SEARCH_NOSHOW|SEARCH_CASE|SEARCH_UP,S)
			if(start == BADADDR):
				break
			if(start < end):
				break
			for byte in R:
				PatchByte(start,int(byte,16))
				start += 1
			start -= len(R)
			found += 1			
	else:
		...
		while(1):
			start = FindBinary(start,SEARCH_NOSHOW|SEARCH_CASE|SEARCH_DOWN,S)
			if(start == BADADDR):
				break
			if(start > end):
				break
...
def main():
	...
	start = int(start,16)
	if(not isLoaded(start)):
		print "start address %08X not load\n" % start
		return
